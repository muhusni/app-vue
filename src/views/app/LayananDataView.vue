        ...
        <v-card-text>
            <v-row class="d-flex justify-center">
                <v-col cols="12" sm="6" class="mx-6">
                    <v-form @submit.prevent="downloadData">
                        ...
                        <v-text-field v-if="formData.chosenData.fl_start_date" label="Tanggal Awal" type="date"
                            v-model="formData.start_date" variant="outlined"></v-text-field>
                        <v-text-field v-if="formData.chosenData.fl_end_date" label="Akhir Awal" type="date"
                            v-model="formData.end_date" variant="outlined"></v-text-field>
                        <v-text-field v-if="formData.chosenData.fl_parameter" :label="formData.chosenData.parameter_name"
                            type="text" v-model="formData.parameter" variant="outlined"></v-text-field>
                        <div class="d-flex justify-center">
                            <v-btn class="my-auto" type="submit" color="success">Download
...
const downloadData = async () => {
    const response = await axios.post('http://192.168.146.99:5000/download_query_result', formData, {
